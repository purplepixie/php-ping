...
class PPPing
{
	...
	function Ping()
	{
		...
		if ($keepon) // didn't time out - read data
		{
			...
			if ((($rx_parts[1] == 0) || (($rx_parts[1] == 8) && ($match))) && ($rx_parts[2] == 0)) { // is echo_reply (0) or is echo_request (8) AND match (from same host)
				// and has code of 0
				// valid response
